...
void iniciarSesion(HashMap *usuarios) {
    char name[15];
    char password[11];

    system("cls");
    mostrarEscenario(50,20);
    gotoxy(18,4);
...
void menu() {
    HashMap *usuarios = createMap(10);
    Pixel pos;
    int time = 0;

    leerArchivoUsuarios(usuarios);

    sndPlaySound("sound\\Plants vs Zombies Soundtrack. [Main Menu].wav", SND_ASYNC);
    pos.X = 8;
    pos.Y = 11;
    system("cls");
    mostrarCursor(true);
    ...
    while (true) {
        ...
        if (time == 10) {
            ...
            if (GetAsyncKeyState(ENTER)) {
                ...
                if (pos.X == 30 && pos.Y == 11) iniciarSesion(usuarios);
